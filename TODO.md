...
- [ ] **Respect light/dark mode in the transcript.** Transcript background is currently hard-coded `Color.white`; switch to `Color(.textBackgroundColor)` so it adapts. Windows in dark mode currently show black text on white, which is jarring.
- [ ] **Show MLX download progress.** First-time `#huggingFaceLoadModelContainer` can sit for minutes with just `"Downloading default…"` in the status line. `ModelContainer` loaders accept a progress callback — wire it to a determinate `ProgressView` in the header.
- [ ] **Cancel in-flight model load.** Right now `Load` is fire-and-forget; no way to abort a stuck HF download or a slow `.gguf` mmap. Add a cancel affordance that flips back to the pre-load state.
- [ ] **System prompt / instructions field.** `ChatSession` and the llama wrapper both accept a system message; expose a small settings popover (persisted in `UserDefaults`).
- [ ] **Generation parameters UI.** Expose `temperature`, `top_p`, `max_tokens`. Today both backends use hardcoded defaults; the llama runner uses `maxTokens: 512` and a fixed sampler chain.
...
- [ ] **CI.** GitHub Actions matrix: `make test` (covers CMake + SPM today) on macos-14, plus a lint step. Leave `build-infer` out of CI initially — it needs the ~700 MB Metal Toolchain asset and HF downloads.
